...
//Proportional Entropy Telescoping Multi-Scalar Grid - PetMSG = petsalt = petrock
public class PetRock {
	...
	public void apply_kernel(float[][] kernel, float scale, float fx, float fy) {
		fx /= resolution;
		fy /= resolution;
		for(int d = 0; d < depth; d++) {
			...
			for( ; cy < y+kernel_size && cy < data.length; cy++) {
				...
				for( ; cx < x+kernel_size && cx < data[0].length; cx++) {
					data[d][cy][cx] += kernel[ky][kx]*scale;
					kx++;
				}
				ky++;
			}
	...
	public float getScalar(float fx, float fy) {
		fx /= resolution;
		fy /= resolution;
	...
	public PetRock(int width, int height, int resolution, int depth) {
		this.depth = depth;
		this.width = width;
		this.height = height;
	...
	public float[] interpolate( int x, int y) {
		...
		dx*=resolution;
		dy*=resolution;
		return bilinear_interpolation(x1,y1,dx,dy,false); //scalar, gradx, grady
	}
	
	float[] bilinear_interpolation(float xl, float yl, float dx, float dy, boolean invert) {
		//scalar,gradx,grady
		...
		if( xl < 0) xl = 0;
		if( yl < 0) yl = 0;
		float x0y0 = getScalar((float)yl,(float)xl);//grid[yl][xl];
		float x1y0 = getScalar((float)yl,(float)xl+resolution);
		float x0y1 = getScalar((float)yl+resolution,(float)xl);
		float x1y1 = getScalar((float)yl+resolution,(float)xl+resolution);
		if( invert) {
			x0y0 = 1.0f/x0y0;
			x0y1 = 1.0f/x0y1;
			x1y0 = 1.0f/x1y0;
	...
	public void update(Entity r,float amt) {
		...
		y = y < 0 ? 0 : y > h ? h : y;
		//if( true) {
		this.apply_kernel(one_over_r_kernel, amt*density_multiplier, x, y);
			//return;
		//}
